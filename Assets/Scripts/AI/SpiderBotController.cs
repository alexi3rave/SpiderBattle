...
namespace WormCrawlerPrototype.AI
{
    public sealed class SpiderBotController : MonoBehaviour
    {
        ...
        private bool _allowHorizontalRopeOnce;
        ...
        [SerializeField] private float minFireAngleFromHorizonDeg = 20f;
        ...
        private IEnumerator RopeBlockedSequenceRoutine(Transform target, int myTeam, GrappleController grapple, WormAimController aim, Vector2 standPos)
        {
            if (target == null || grapple == null)
            {
                yield break;
            }

            bool IsNowInRangeToShoot()
            {
                var origin = aim != null ? aim.AimOriginWorld : (Vector2)transform.position;
                var targetPoint = GetTargetAimPoint(target);
                var heroH0 = GetHeroHeight();
                ...
                GetShootFlags(origin, target, myTeam, grenadeExplosionRadius, effectiveClawRange, effectiveGrenadeRange, out var canClaw, out var canGrenade, out _);
                return canClaw || canGrenade;
            }

            var heroH = GetHeroHeight();
            var minMove = Mathf.Max(0.25f, heroH * 3f);
            var stuckEpsMove = Mathf.Max(0.10f, heroH * 0.60f);
            ...
            if (IsNowInRangeToShoot())
            {
                yield break;
            }

            if (movedDown.Value >= minMove)
            {
                yield break;
            }
        }

        private IEnumerator RopeVerticalFullExtendRoutine(Transform target, GrappleController grapple, WormAimController aim, Vector2 ropeDir, FloatHolder movedOut)
        {
            if (grapple == null || target == null || movedOut == null)
            {
                yield break;
            }
            ...
            if (!TryFireRopeValidated(grapple, aim, ropeDir, RopeFirePolicy.General))
            {
            ...
            while (grapple.IsAttached)
            {
                ...
                if (Time.time - checkT >= 2.0f)
                {
                    var o2 = aim != null ? aim.AimOriginWorld : (Vector2)transform.position;
                    var d = (GetTargetAimPoint(target) - o2).magnitude;
                    if (d >= (prevDist - 0.05f))
                    {
                        break;
                    }
            ...
            yield return SafeDetachRopeRoutine(grapple, anchorLikelyBelow: ropeDir.y < -0.5f);
            if (aim != null) aim.SetExternalAimOverride(false, Vector2.down);

            yield return new WaitForSeconds(Mathf.Max(0.0f, ropePostManeuverRecheckDelay));
            var shot = new BoolHolder();
            yield return TryShootIfPossibleNowRoutine(target, grapple, aim, GetComponent<HeroSurfaceWalker>(), shot);
            if (shot.Value)
            {
                yield break;
            }
        ...
        private IEnumerator RopeHorizontalHalfLengthEscapeRoutine(Transform target, GrappleController grapple, WormAimController aim, FloatHolder movedOut)
        {
            // Horizontal rope shots (0/180) are forbidden for bots.
            if (movedOut != null) movedOut.Value = 0f;
            yield break;
        }
        ...
        private IEnumerator RopeUpSwingAndLowerRoutine(Transform target, GrappleController grapple, WormAimController aim, FloatHolder movedOut)
        {
            if (grapple == null || target == null || movedOut == null)
            {
                yield break;
            ...
            // Swing toward enemy and remember the closest distance.
            var bestDist = float.PositiveInfinity;
            var swingStartT = Time.time;
            while (grapple.IsAttached && Time.time - swingStartT < 1.0f)
            {
                var o2 = aim != null ? aim.AimOriginWorld : (Vector2)transform.position;
                var d = (GetTargetAimPoint(target) - o2).magnitude;
                ...
                // Once we are no longer improving, start lowering.
                if (d > bestDist + 0.05f)
                {
                    break;
                }
            ...
            // Lower down (extend rope) when at closest, then detach safely.
            var maxLen = grapple.MaxRopeLength;
            var lowerStartT = Time.time;
            while (grapple.IsAttached && Time.time - lowerStartT < 1.0f)
            {
                if (maxLen > 0.0001f && grapple.CurrentRopeLength >= maxLen - 0.25f)
                {
                    break;
                }
                grapple.SetExternalMoveOverride(true, moveH: 0f, moveV: -1f);
            ...
            grapple.SetExternalMoveOverride(false, 0f, 0f);
            yield return SafeDetachRopeRoutine(grapple, anchorLikelyBelow: false);
            if (aim != null) aim.SetExternalAimOverride(false, Vector2.down);
        ...
        private IEnumerator RopeForwardHalfLengthRoutine(Transform target, GrappleController grapple, WormAimController aim, FloatHolder movedOut)
        {
            // Horizontal rope shots (0/180) are forbidden for bots. Use walking for horizontal movement.
            if (movedOut != null) movedOut.Value = 0f;
            yield break;
            ...
            // Horizontal rope shots are only allowed as a one-shot fallback after a failed -90 pull-up.
            if (!_allowHorizontalRopeOnce)
            {
                movedOut.Value = 0f;
                yield break;
            }
            _allowHorizontalRopeOnce = false;
            ...
            var targetPoint = GetTargetAimPoint(target);
            var sign = (targetPoint.x - origin.x) >= 0f ? 1f : -1f;
            var ropeDir = new Vector2(sign, 0f);
            if (ropeDir.sqrMagnitude < 0.0001f) ropeDir = Vector2.right;
            ropeDir.Normalize();
            if (aim != null) aim.SetExternalAimOverride(true, ropeDir);
            ...
            // Even though this routine is currently disabled (early yield break), keep rope firing guarded.
            if (!TryFireRopeValidated(grapple, aim, ropeDir, RopeFirePolicy.General))
            {
                if (walker != null) walker.SetIdleAnchorLock(false);
                movedOut.Value = ((Vector2)transform.position - startPos).magnitude;
            ...
            var half = Mathf.Max(1.0f, grapple.MaxRopeLength * 0.5f);
            var startT = Time.time;
            while (grapple.IsAttached && Time.time - startT < 1.0f)
            {
                if (grapple.CurrentRopeLength >= half - 0.25f)
                {
                    break;
                }
                grapple.SetExternalMoveOverride(true, moveH: 0f, moveV: -1f);
        ...
        private IEnumerator RopeBackHalfLengthRoutine(Transform target, GrappleController grapple, WormAimController aim, FloatHolder movedOut)
        {
            // Horizontal rope shots (0/180) are forbidden for bots. Use walking for horizontal movement.
            if (movedOut != null) movedOut.Value = 0f;
            yield break;
            ...
            var sign = (targetPoint.x - origin.x) >= 0f ? 1f : -1f;
            var ropeDir = new Vector2(-sign, 0f);
            if (ropeDir.sqrMagnitude < 0.0001f) ropeDir = Vector2.left;
            ropeDir.Normalize();
            if (aim != null) aim.SetExternalAimOverride(true, ropeDir);
        ...
        private IEnumerator RopeFixed45CrawlRoutine(Transform target, GrappleController grapple, WormAimController aim, FloatHolder movedOut)
        {
            if (grapple == null || target == null)
            {
                yield break;
            }
            ...
            // 45 degrees from horizon toward enemy.
            var ropeDir = new Vector2(sign, 1f).normalized;
            if (aim != null) aim.SetExternalAimOverride(true, ropeDir);
            ...
            // Crawl up a bit and swing toward enemy.
            grapple.SetExternalMoveOverride(true, moveH: sign, moveV: 1f);
            yield return new WaitForSeconds(Mathf.Max(0.05f, approachStepSeconds));
            grapple.SetExternalMoveOverride(true, moveH: sign, moveV: 0f);
            yield return new WaitForSeconds(Mathf.Max(0.05f, approachStepSeconds));
        ...
        private IEnumerator RopeFallbackVerticalSwingRoutine(Transform target, GrappleController grapple, WormAimController aim)
        {
            if (grapple == null || target == null)
            {
                yield break;
            ...
            // Fallback: shoot rope under feet (down) for no-ceiling / no-hook scenarios.
            var ropeDir = Vector2.down;
            if (aim != null)
            {
            ...
            // Under-feet rope: to "push off" the ground, reel in briefly to build tension while ground blocks downward motion.
            // Then extend rope toward max length so the hero can rise into the air on the rope.
            grapple.SetExternalMoveOverride(true, moveH: 0f, moveV: 1f);
            yield return new WaitForSeconds(Mathf.Min(Mathf.Max(0.05f, maxVerticalRopePhaseSeconds), Mathf.Max(0.05f, ropeFallbackJumpReelSeconds)));

            var maxLen = grapple.MaxRopeLength;
            var extendStartT = Time.time;
            while (grapple.IsAttached)
            {
                if (Time.time - extendStartT >= Mathf.Max(0.05f, maxVerticalRopePhaseSeconds))
                {
                    break;
                }
                if (maxLen > 0.0001f && grapple.CurrentRopeLength >= maxLen - 0.25f)
            ...
            grapple.SetExternalMoveOverride(true, moveH: sign, moveV: 0f);
            yield return new WaitForSeconds(Mathf.Max(0.05f, ropeFallbackSwingSeconds));
        ...
        private IEnumerator TryShootFromRopeIfPossibleHardRoutine(Transform target, int myTeam, GrappleController grapple, WormAimController aim, BoolHolder shot)
        {
            if (shot != null) shot.Value = false;
            ...
            if (target == null || grapple == null || aim == null)
            {
                yield break;
            }
            ...
            var origin = aim.AimOriginWorld;
            var targetPoint = GetTargetAimPoint(target);

            var heroH0 = GetHeroHeight();
            var grenadeExplosionRadius = Mathf.Max(0.25f, heroH0 * 2.5f);

            var effectiveGrenadeRange = Mathf.Max(0.1f, grenade.GetMaxRangePublic());
            var effectiveClawRange = Mathf.Max(0.1f, effectiveGrenadeRange * 1.5f);

            GetShootFlags(origin, target, myTeam, grenadeExplosionRadius, effectiveClawRange, effectiveGrenadeRange, out var canClaw, out var canGrenade, out var dist);
            if (!canGrenade)
            {
                yield break;
            }

            var closeClaw = dist <= Mathf.Max(0.75f, heroH0 * 1.25f);
            var weapon = SelectWeaponForTeamBalance(myTeam, canClaw, canGrenade, closeClaw);
            if (weapon != TurnManager.TurnWeapon.Grenade)
            {
                yield break;
            ...
            if (!TryFindGrenadeAim(grenade, targetPoint, myTeam, out var gAim))
            {
                yield break;
            }
            ...
            grenade.Enabled = true;
            aim.SetExternalAimOverride(true, gAim);
            yield return new WaitForSeconds(0.10f);
        ...
        private IEnumerator RopeWellEscapeRoutine(Transform target, GrappleController grapple, WormAimController aim)
        {
            if (grapple == null || target == null)
            {
                yield break;
            ...
            // Escape "wells": shoot rope under feet, reel in to climb/jump up, then swing/fall while attached toward enemy.
            var ropeDir = Vector2.down;
            if (aim != null)
            {
                aim.SetExternalAimOverride(true, ropeDir);
            ...
            // Same as fallback: reel-in briefly to push off, then extend to rise on full rope length.
            grapple.SetExternalMoveOverride(true, moveH: 0f, moveV: 1f);
            yield return new WaitForSeconds(Mathf.Min(Mathf.Max(0.05f, maxVerticalRopePhaseSeconds), Mathf.Max(0.05f, ropeFallbackJumpReelSeconds)));
            ...
            // Swing while staying on rope; this acts like "safe fall" toward enemy.
            grapple.SetExternalMoveOverride(true, moveH: sign, moveV: 0f);
            yield return new WaitForSeconds(Mathf.Max(0.05f, ropeFallbackSwingSeconds));
        ...
        private IEnumerator RopeTowardPointRoutine(Vector2 point, int myTeam, GrappleController grapple, WormAimController aim)
        {
            if (grapple == null)
            {
                yield break;
            ...
            if (toPoint.sqrMagnitude < 0.0001f) toPoint = Vector2.right;
            var signTo = toPoint.x >= 0f ? 1f : -1f;

            var stuck = false;
            var ropeDir = FindBestRopeDir(origin, toPoint, stuck, attempt: 0);
            if (aim != null) aim.SetExternalAimOverride(true, ropeDir);
            ...
            if (walker != null) walker.SetIdleAnchorLock(false);
            if (!grapple.IsAttached) yield break;

            grapple.SetExternalMoveOverride(true, moveH: signTo, moveV: 1f);
            yield return new WaitForSeconds(ropeReelSeconds * 0.75f);
            grapple.SetExternalMoveOverride(true, moveH: signTo, moveV: 0f);
            yield return new WaitForSeconds(ropeSwingSeconds * 0.6f);

            grapple.SetExternalMoveOverride(false, 0f, 0f);
            yield return SafeDetachRopeRoutine(grapple, anchorLikelyBelow: false);
            if (aim != null) aim.SetExternalAimOverride(false, Vector2.down);

            // No target info here; shooting is handled by the caller after moving.
        }
        ...
        private Vector2 ClampAimForFiring(Vector2 aimDir, Vector2 toTarget)
        {
            if (aimDir.sqrMagnitude < 0.0001f)
            {
                var sx0 = toTarget.x >= 0f ? 1f : -1f;
                return new Vector2(sx0, 1f).normalized;
            }

            aimDir.Normalize();

            var absX = Mathf.Abs(aimDir.x);
            var absY = Mathf.Abs(aimDir.y);
            var angleFromHorizon = Mathf.Atan2(absY, absX) * Mathf.Rad2Deg;
            if (angleFromHorizon < Mathf.Max(0f, minFireAngleFromHorizonDeg))
            {
                var sx = toTarget.x >= 0f ? 1f : -1f;
                return new Vector2(sx, 1f).normalized;
            }
